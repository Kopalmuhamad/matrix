...
async function fetchProducts() {
    try {
        const res = await fetch('http://localhost:3000/api/products?category=pants', { cache: 'no-store' });
        if (!res.ok) {
...
const PantsSection = async () => {
    ...
    return (
        <div className='pt-[60px]'>
            <Container>
                <header className='py-10 text-2xl md:text-3xl lg:text-4xl font-semibold text-foreground'>Pants</header>
